...
pub fn resolve_predicate(predicate: Predicate, tx: &Transaction) -> bool {
    match predicate {
        ...
        Predicate::Unary(bool_op::Unary::Not, x) => !resolve_predicate(*x, tx),
...
mod tests {
    ...
    fn test_bool_op() {
        ...
        // Contrapositive
        let pred_and = Predicate::Unary(bool_op::Unary::Not, Box::new(Predicate::Constant(false)));
        assert!(resolve_predicate(pred_and, &dummy));

        let pred_and = Predicate::Unary(bool_op::Unary::Not, Box::new(Predicate::Constant(true)));
        assert!(!resolve_predicate(pred_and, &dummy));
    }
}
